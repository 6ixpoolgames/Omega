...
4. [Future Field Atlas compact manageability H64](docs/research_notes/validation_results/future_field_atlas_compact_manageability_h64_result.md)
5. [Future Field Atlas default-gzip compression smoke](docs/research_notes/validation_results/future_field_atlas_default_gzip_compression_smoke.md)
6. [Future Field Atlas H128 calibration pass](docs/research_notes/validation_results/future_field_atlas_h128_calibration_pass_result.md)
7. [Coupled H128 depth and triadic profile smoke](docs/research_notes/validation_results/future_field_atlas_coupled_h128_and_triadic_profile_smoke_result.md)
8. [Future Field Atlas glossary](docs/FUTURE_FIELD_ATLAS_GLOSSARY.md)
9. [Hard top-m mechanism result](docs/research_notes/validation_results/rfs_mb0_top_m_mechanism_audit_result.md)
10. [Top-m geometry audit result](docs/research_notes/validation_results/rfs_mb0_top_m_geometry_audit_result.md)
11. [MaxEnt local transition Phase 1 preflight result](docs/research_notes/validation_results/rfs_mb0_max_entropy_local_transition_phase1_preflight_result.md)
12. [Low-beta preservation sensitivity scaleup result](docs/research_notes/validation_results/rfs_mb0_low_beta_preservation_sensitivity_scaleup_result.md)
13. [Asymmetry-ladder preservation scaleup result](docs/research_notes/validation_results/rfs_mb0_asymmetry_ladder_preservation_scaleup_result.md)
14. [Horizon Transport and Aligned Amplification](docs/research_notes/omega_theory/horizon_transport_aligned_amplification.md)
15. [Transition Energy and Constraint Untethering](docs/research_notes/omega_theory/transition_energy_and_constraint_untethering.md)
16. [Transition-Energy Substrate Atlas](docs/research_notes/omega_theory/transition_energy_substrate_atlas.md)
...
VAL0/VAL1 were useful reconnaissance probes. RFS-MB0 reoriented from endpoint support/distribution deformation and static coflow spectra toward directional **horizon-transport** instrumentation. The H128 horizon-transport response-surface scaleup passed matched-marginal instrument gates and surfaced a stable-to-amplified-aligned response surface. Transition-energy characterization then turned the branch into a substrate-response atlas. The asymmetry-ladder and low-beta runs made preservation asymmetry the current loadbearing substrate ingredient, especially `symbol_histogram_distance`; the MaxEnt and top-m audits then narrowed that read. The latest hard-top-m mechanism pass found a fixed low-rank successor-boundary pattern: rank-prefix `m=3`, rank-subset `m=4` retaining ranks `1;2;3`, and rank-subset `m=5` retaining ranks `1;2;3` were response-bearing, while expansion, random deletion, and strongest-edge deletion stayed stable. The current implementation branch is now a **Future Field Atlas** clean slate: raw reachable-frontier topology first, response labels last, and condition identity expressed through state-space, transition-law, selection-operator, observable, and frontier-scan specs. The single-field H128 calibration pass completed 128/128 scans with complete artifacts and passing reconstruction audits at about 172.7 MB total output. The coupled H128 pair2 serial depth gate also completed cleanly with complete artifacts and passing audits at about 0.746 GiB, while the H128 pair4 parallel attempt exposed a Windows multiprocessing result-transfer limit on heavy pairs. Historical treatment-arm names are documented only for translation in the atlas glossary, not used as runtime schema. The current empirical target is therefore shared low-rank successor-boundary pressure for future coupled-frontier work, not generic lower out-degree or a broad Omega claim. Broad H128 coupled surveys are blocked on worker-side spooling or pair-indexed heavy-pair handling; the new triadic path is only a profile-level interface smoke. The formal core has now been closed at v0 as a boundary-nonprivileged definition of Omega-compatible viable propagation. This is still not a scientific validation gate, candidate promotion, holdout result, or Omega/agency/value claim.
