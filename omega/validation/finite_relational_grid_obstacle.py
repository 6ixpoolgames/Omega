"""Controlled gridworld obstacle-insertion validation."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the controlled gridworld obstacle-insertion pilot."
    )
    parser.add_argument("--out-root", type=Path, default=Path(".tmp/finite_relational_grid_obstacle"))
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    result = run_finite_relational_grid_obstacle(out_root=args.out_root)
    print(json.dumps(result, indent=2, sort_keys=True, default=str))


def run_finite_relational_grid_obstacle(
    *,
...
def _retain_case(case: GridObstacleCase, out_dir: Path) -> dict[str, Any]:
