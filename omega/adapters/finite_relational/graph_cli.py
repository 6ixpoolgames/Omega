"""CLI for compiling and auditing derived graph adapter sources."""
...
from omega.adapters.finite_relational.derived_graph import compile_derived_graph_path, load_derived_graph_path
from omega.adapters.finite_relational.model import load_model, model_digest, validate_provenance


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compile and audit a derived graph adapter source.")
...
def main() -> None:
    args = parse_args()
    summary = run_graph_file(args.source, args.out)
    print(json.dumps(summary, indent=2, sort_keys=True))


def run_graph_file(source_path: Path, out_dir: Path) -> dict[str, object]:
    source = load_derived_graph_path(source_path)
    ...
    summary = {
        "source_model_id": str(source.get("model_id", "derived_graph")),
