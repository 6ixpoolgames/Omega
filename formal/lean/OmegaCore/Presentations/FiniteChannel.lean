...
Finite channel / partition presentation for Omega Primitive Calculus v0.
...
This is a worked presentation of the root calculus. It is not probabilistic
Shannon theory, an empirical adapter, Future Field Atlas semantics,
...
/-- Observable-distinction preorder frame. -/
def obsDistFrame (X : Type u) : PreorderFrame (ObsDist.{u, v} X) where
  le := Refines
...
/-- Channel-induced exact-recovery transport. -/
def channelTransport
    {X : Type u} {Y : Type w}
    (K : X -> Y -> Prop) :
    DistTransport (obsDistFrame (X := X)) (obsDistFrame (X := Y)) where
  rel := ExactRecovers K
  closed := by
...
/-- Channel-induced transport is lax over channel composition. -/
theorem channelTransport_comp_subset
    {X : Type u} {Y : Type w} {Z : Type z}
    (K : X -> Y -> Prop)
    (L : Y -> Z -> Prop) :
    DistTransport.Subset
      (DistTransport.compose (channelTransport K) (channelTransport L))
