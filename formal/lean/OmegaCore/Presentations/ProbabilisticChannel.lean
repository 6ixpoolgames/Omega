...
/-!
Finite cascade composition.

The cascade layer intentionally uses path-level natural weights. Its first- and
second-stage error masses are measured over the same finite path ensemble as the
composite decoder error. They are not standalone independently normalized stage
errors.
-/
...
theorem cascadeTotalMass_eq_totalMass_chanComp
  ...
  intro x _hx
  calc
    (∑ y, ∑ z, pi x * K x y * L y z)
        = ∑ z, ∑ y, pi x * K x y * L y z := by
          rw [Finset.sum_comm]
    _ = ∑ z, pi x * ∑ y, K x y * L y z := by
          apply Finset.sum_congr rfl
          intro z _hz
          rw [Finset.mul_sum]
          apply Finset.sum_congr rfl
...
theorem cascadeCompositeErrorMass_eq_errorMass_chanComp
  ...
  intro x _hx
  calc
    (∑ y, ∑ z, if dec1 (dec2 (F z)) = D x then 0 else pi x * K x y * L y z)
        =
      ∑ z, ∑ y, if dec1 (dec2 (F z)) = D x then 0 else pi x * K x y * L y z := by
        rw [Finset.sum_comm]
    _ =
      ∑ z, if dec1 (dec2 (F z)) = D x then 0 else pi x * ∑ y, K x y * L y z := by
        apply Finset.sum_congr rfl
        intro z _hz
    ...
    _ =
      pi x * ∑ z, if dec1 (dec2 (F z)) = D x then 0 else ∑ y, K x y * L y z := by
        rw [Finset.mul_sum]
        apply Finset.sum_congr rfl
        intro z _hz
        by_cases hCorrect : dec1 (dec2 (F z)) = D x
