# Valuerhood as Recoverable Historical Identity
...
The previous notes established:
...
> What kind of process can be a bearer of value-preservation constraints at all?
...
## What valuerhood is not

Valuerhood is not identical to consciousness.

It is not identical to explicit preference.

It is not identical to utility maximization.

It is not identical to reward optimization.

It is not identical to intelligence.

It is not identical to moral patienthood.
...
A valuer requires a boundary in the minimal functional sense.

A boundary is not necessarily a hard wall, membrane, skin, or explicit interface. It may be dynamic, probabilistic, porous, or distributed.

What matters is that the process maintains enough distinction between itself and its environment for continuation to be meaningful.

Without boundary, there is only undifferentiated dynamics. There is no process whose continuation can be preserved or lost.

Boundary supports:

```text
identity:
  something can count as the same continuing process

perturbation:
  something can be disturbed

repair:
  something can recover

interaction:
  exchange with environment can preserve or degrade continuability
```
...
Recoverability is the floor that separates cheap persistence from proto-value.

A system that persists only by stasis is too brittle.

A system that persists only by a clock cycle is too compressible.

A system that persists only by lock-in has collapsed its future possibility.

A valuer-like process must remain continuable under perturbation.

Recoverability does not mean exact restoration. Exact restoration would often be another fakeout. Recoverability means that after disturbance, there remains a continuation path that preserves the relevant identity class.
...
This is why recoverability is the FEP-like floor. The process resists dispersion not because it is static, but because it can maintain identity through disturbance and exchange.
...
adaptive response
model-guided action
cooperative corridor formation
```
...
Valuerhood requires that different continuations matter structurally for the process.

This does not yet mean moral value. It means path consequence:

```text
some continuations preserve recoverable identity
some degrade it
some expand future continuability
some collapse it
some permit repair
some cross irreversible horizons
```

At this point, continuation and collapse are no longer neutral descriptions. They become structurally non-equivalent relative to the process's own continuability.
...
## Non-valuers, proto-valuers, agents

The following distinctions are useful but not sharp metaphysical boundaries.
...
examples:
  minimal homeostatic systems, autopoietic toy systems, simple self-maintaining organisms
```

```text
agent:
  a valuer with action channels that modulate its future continuability

examples:
  systems that select actions, regulate behavior, or use internal states to affect future viability
```

```text
reflective / cognitive valuer:
  a valuer whose internal dynamics model, represent, or explicitly weight possible futures

examples:
  active inference systems, learning agents, animals, humans, organizations with self-models
```

Omega does not require all valuers to be reflective. But the richer Omega questions become more salient as valuerhood becomes more agentic, compositional, and model-bearing.

## Relation to Gradient Ethics
...
Omega is not the theory of valuerhood alone.

Valuerhood defines the locus where value-bearing can begin. Omega studies the trajectory structure of such loci under constraint.

Once valuers exist, the older Omega machinery becomes relevant:

```text
recoverable reachability:
  which futures remain accessible without irreversible collapse?

distinguishable viable futures:
  how rich is the space of structurally distinct continuations?

bounded cost:
  what trajectories remain physically and computationally feasible?

epistemic grounding:
  which models remain coupled to reality enough to guide continuation?

scale consistency:
  which abstractions preserve real viability rather than inventing fake viability?

inter-history compatibility:
  how do valuers interact without destroying each other's recoverable continuability?

mutual corridors:
  which interactions preserve or expand compatible future passage?

pseudo-Omega attractors:
  which locally persistent structures degrade broader value-bearing reachability?

redemption / re-entry:
  when can damaged histories reconnect to Omega-compatible trajectories?
```
...
The validation question should become:

> Given valuers, do Omega constraints predict which trajectories persist, collapse, recover, become trapped, or remain mutually compatible better than simpler baselines?
