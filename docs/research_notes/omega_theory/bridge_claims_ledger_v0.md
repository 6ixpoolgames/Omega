...
| Admissibility as fact/presentation closure. | Soundness and hidden-loss checks | Boundary-invariant facts | ACTIVE TARGET | Generic Galois closure, X2 coordinate, recovery, and viability collapse pilots are landed; still need carrier instantiation and adversarial generated collapse tests. | `PresentationFactViabilityExamples.lean` shows exact viability-status facts survive exact-status presentations but disappear when a constant status presentation is added. | Honest admissibility families collapse all recovery/viability facts to constants. | Does not prove a valuer, agency, or Omega object. |
