...
| Recovery failure persists under deterministic coarsening. | Observation refinement theorem | Irreversible loss / Gradient Ethics bridge | ACTIVE TARGET | Prove finite coarsening-chain theorem and restricted/robust variants. | If `o_{n+1} = g_n after o_n`, failure at `n` persists to later `m`. | A deterministic coarsening that restores a failed unrestricted recovery threshold. | Does not cover new measurements, new side information, or non-deterministic observation changes. |
| Admissibility as fact/presentation closure. | Soundness and hidden-loss checks | Boundary-invariant facts | PILOT TARGET | Define finite `presentation satisfies fact`, compute `Adm`/`Inv` closures, adversarially test collapse. | Nonconstant recovery/viability facts survive all intrinsically admissible presentations in a toy substrate. | Honest admissibility families collapse all facts to constants. | Does not prove a valuer, agency, or Omega object. |
| Sound transport preserves recovery/viability facts. | Presentation transport | Decomposition-invariance | ACTIVE TARGET | Define transport-compatible fact preservation once the fact surface is fixed. | Recovery or viability fact transported across two incomparable sound presentations. | Sound-looking transport that fabricates or hides recovery/viability. | Does not claim all presentations are comparable. |
