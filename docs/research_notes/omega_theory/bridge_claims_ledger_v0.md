...
| Robust joint viability under correlated shocks. | Robust recovery / policy continuation | Distributed agency constraint | PILOT LANDED | Formalize stable statements only after more adapter controls. | `omega.adapters.finite_relational.stochastic_policy_dynamics` now includes a retained exact-rational policy family where target A and target B each have robust policies, but no declared policy robustly attains the joint target under a correlated shock. | Joint robustness always follows from individual robust attainability under declared assumptions. | Does not prove distributed agency is always better, does not validate real policy safety, and does not define value, agency, or Omega. |
