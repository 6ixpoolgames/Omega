...
| Admissibility as fact/presentation closure. | Soundness and hidden-loss checks | Boundary-invariant facts | ACTIVE TARGET | Generic Galois closure and an X2 coordinate-collapse pilot are landed; still need recovery/viability instantiation and adversarial collapse tests. | `PresentationFactClosureExamples.lean` shows first-coordinate target and visible-pair facts survive first-only presentations but disappear when second-coordinate presentations are added. | Honest admissibility families collapse all recovery/viability facts to constants. | Does not prove a valuer, agency, or Omega object. |
