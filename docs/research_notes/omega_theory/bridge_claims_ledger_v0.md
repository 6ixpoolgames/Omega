...
| Admissibility as fact/presentation closure. | Soundness and hidden-loss checks | Boundary-invariant facts | ACTIVE TARGET | Generic Galois closure, X2 coordinate, recovery, viability, and carrier collapse pilots are landed; still need adapter-generated or adversarial collapse tests. | `PresentationFactCarrierExamples.lean` shows a certified recurrent carrier pair survives exact endpoint-status presentation but disappears from common visible-pair facts when a constant status presentation is added. | Honest admissibility families collapse all recovery/viability/carrier facts to constants or empty visible-pair sets. | Does not prove a valuer, agency, carrier identity, or Omega object. |
