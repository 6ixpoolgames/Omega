...
| Admissibility as fact/presentation closure. | Soundness and hidden-loss checks | Boundary-invariant facts | ACTIVE TARGET | Generic Galois closure, X2 coordinate, recovery, viability, carrier, stale/reflected, multi-presentation, and first transport-aware generated pilots are landed; still need empirical-adjacent and transport-failure generated closure tests. | The finite relational adapter now generates closure shrinkage cases for a derived carrier pair, transition-derived reachability status, transition/safety-derived viability status, bounded-recovery observation status, stale/reflected reach-status facts, row/column multi-presentation families, and transported endpoint-role facts under a transfer contract. | Honest admissibility families collapse all recovery/viability/carrier/transport facts to constants or empty visible-pair sets. | Does not prove a valuer, agency, carrier identity, or Omega object. |
| Sound transport preserves recovery/viability facts. | Presentation transport | Decomposition-invariance | ACTIVE TARGET | First adapter transport-closure pilot landed; still need formal transport-compatible fact preservation once the fact surface is fixed. | `generated_transport_fact_closure` certifies a carrier transfer and checks that lifted source/target role presentations preserve a transported endpoint-role fact. | Erasing or failed transport views remove the transported fact from common facts or fabricate a transport-looking role without carrier transfer. | Does not claim all presentations are comparable or that transfer is identity. |
