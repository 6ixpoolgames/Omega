...
| Alpha traces do not canonically determine agency. | Alpha | Agency realization | ACTIVE TARGET | Prove a finite underdetermination witness: same Alpha frame with inequivalent passive and feedback-maintained agency realizations. | Candidate witness should share relation/distinction/asymmetry structure while changing the realized control/feedback/maintenance surface. | If Alpha uniquely fixed the agency realization, the two-realization witness would be impossible. | Does not say Alpha is irrelevant; it says lifting is noncanonical. |
...
| Coarse continuation does not certify protected fine continuation. | Target postprocessing / nonfactorization | Protected plurality / lineage | ACTIVE TARGET | Package existing target-postprocessing positive theorem with a nonreflection/nonfactorization theorem and protected-family interpretation. | Fine target preservation implies coarse target preservation; non-injective coarse target merges fine protected states, so macro preservation cannot certify fine preservation. | If every coarse target recovery certified fine target recovery without reflection. | Does not define personal identity, moral standing, or valuerhood. |
