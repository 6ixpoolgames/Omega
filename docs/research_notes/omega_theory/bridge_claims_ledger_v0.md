...
| Admissibility as fact/presentation closure. | Soundness and hidden-loss checks | Boundary-invariant facts | ACTIVE TARGET | Generic Galois closure, X2 coordinate, recovery, viability, carrier, and first adapter-generated carrier-pair closure pilots are landed; still need broader generated/adversarial closure tests for recovery, reachability, and viability facts. | `generated_presentation_fact_closure` in the finite relational adapter derives a carrier certificate from a graph source, keeps the certified pair visible under identity presentation, and removes it from common visible facts when a constant presentation is admitted. | Honest admissibility families collapse all recovery/viability/carrier facts to constants or empty visible-pair sets. | Does not prove a valuer, agency, carrier identity, or Omega object. |
