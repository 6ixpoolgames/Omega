...
| Admissibility as fact/presentation closure. | Soundness and hidden-loss checks | Boundary-invariant facts | ACTIVE TARGET | Generic Galois closure, X2 coordinate, recovery, viability, carrier, stale/reflected, multi-presentation, transport-aware, failed-transport generated pilots, the first grid obstacle source-generator closure pilot, and a stochastic continuation hit-status closure check are landed; still need policy-conditioned and second-source parity closure tests. | The finite relational adapter now generates closure shrinkage cases for a derived carrier pair, transition-derived reachability status, transition/safety-derived viability status, bounded-recovery observation status, stale/reflected reach-status facts, row/column multi-presentation families, transported endpoint-role facts under a transfer contract, a failed-transfer label-closure control, a grid obstacle source-generator case where stale/reflected source-reach status removes the after-reachability fact from common target facts, and a stochastic continuation case where stale/reflected hit-status removes an after-hit threshold fact. | Honest admissibility families collapse all recovery/viability/carrier/transport facts to constants or empty visible-pair sets. | Does not prove a valuer, agency, carrier identity, or Omega object. |
