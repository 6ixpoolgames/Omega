...
| Admissibility as fact/presentation closure. | Soundness and hidden-loss checks | Boundary-invariant facts | ACTIVE TARGET | Generic Galois closure, an X2 coordinate-collapse pilot, and a recovery-collapse pilot are landed; still need viability/carrier instantiation and adversarial collapse tests. | `PresentationFactRecoveryExamples.lean` shows exact bit recovery facts survive exact-only observations but disappear when a constant observation is added. | Honest admissibility families collapse all recovery/viability facts to constants. | Does not prove a valuer, agency, or Omega object. |
