...
The per-property residue vector and family classification are the fixture calibration. The ancestry property encodes route matching, while the other two properties are history-blind, so the retained family-dependent vector was fixed by the declared property family.
